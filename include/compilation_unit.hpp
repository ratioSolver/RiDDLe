...
namespace riddle
{
  ...
  {
  public:
    compilation_unit(std::vector<utils::u_ptr<type_declaration>> &&types, std::vector<utils::u_ptr<method_declaration>> &&methods, std::vector<utils::u_ptr<predicate_declaration>> &&predicates, std::vector<utils::u_ptr<statement>> &&statements) : types(std::move(types)), methods(std::move(methods)), predicates(std::move(predicates)), body(std::move(statements)) {}
  ...
  private:
    ...
    std::vector<utils::u_ptr<statement>> body;                   // The statements.
  };
} // namespace riddle