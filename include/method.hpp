...
namespace riddle
{
  ...
  {
  public:
    method(scope &scp, std::optional<std::reference_wrapper<type>> return_type, std::string_view name, std::vector<utils::u_ptr<field>> &&args = {}, const std::vector<utils::u_ptr<statement>> &body = {}) noexcept;
     ...
     */
    [[nodiscard]] expr invoke(std::shared_ptr<component> self, std::vector<expr> &&args) const;
