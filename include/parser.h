...
namespace riddle
{
  ...
  private:
    ...
    const ast::field_declaration *new_field_declaration(std::vector<id_token> tp, std::vector<utils::u_ptr<const ast::variable_declaration>> ds) const noexcept { return new const ast::field_declaration(std::move(tp), std::move(ds)); }
    const ast::constructor_declaration *new_constructor_declaration(std::vector<std::pair<const std::vector<id_token>, const id_token>> pars, std::vector<id_token> ins, std::vector<std::vector<ast::expression_ptr>> ivs, std::vector<utils::u_ptr<const ast::statement>> stmnts) const noexcept { return new const ast::constructor_declaration(std::move(pars), std::move(ins), std::move(ivs), std::move(stmnts)); }
    ...
    const ast::disjunction_statement *new_disjunction_statement(std::vector<std::vector<utils::u_ptr<const ast::statement>>> conjs, std::vector<ast::expression_ptr> conj_costs) const noexcept { return new const ast::disjunction_statement(std::move(conjs), std::move(conj_costs)); }
    const ast::conjunction_statement *new_conjunction_statement(std::vector<utils::u_ptr<const ast::statement>> stmnts) const noexcept { return new const ast::conjunction_statement(std::move(stmnts)); }
